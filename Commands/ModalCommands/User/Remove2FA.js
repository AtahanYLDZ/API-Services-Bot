...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        let authData = await customerSchema.findOne({ Auth: auth, Password: password })
        if(!authData) {
        ...
        if(!await verify2FA(authData, code)) return await int.update({ embeds:[embed.setDescription(`2FA Kodu yanlis girildi.`)] })

        authData.TwoFactor.active = false;
        authData.TwoFactor.secret = null;
        await authData.save();
