...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        let row = new ActionRowBuilder().addComponents([
            ...
            new ButtonBuilder()
            .setCustomId(`deAktifEt-${auth}-${api}-${int.user.id}`)
            .setEmoji(emojis.no)
        ...
        let returnTime = Date.now() - apiData.returnTimestamp

        apiData.endTimestamp = apiData.endTimestamp + returnTime
        ...
        \`${apiData.name}\` API'niz <t:${Math.floor(Date.now() / 1000)}> tarihi itibariyle kullanıma açık duruma getirilmiştir.
        Bitiş tarihi: <t:${Math.floor(apiData.endTimestamp / 1000)}> (<t:${Math.floor(apiData.endTimestamp / 1000)}:R>)`)], components: [row], ephemeral: true })
